        ...
        try {
            ...
            if (ps.size() > 1) {
                for (int i = 1; i < ps.size(); i++) {
                    ...
                    EntitySpec<?> containerSpec = EntitySpec.create(config().get(DOCKER_CONTAINER_SPEC))
                    ...
                    DockerContainer added = (DockerContainer) getDockerContainerCluster().addChild(containerSpec);
